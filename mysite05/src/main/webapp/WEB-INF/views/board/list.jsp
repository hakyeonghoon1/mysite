			...
			<div id="board">
						...
						<c:forEach items='${list }' var='vo' varStatus='status'>			
							<tr>
								...
								<td style="text-align:left; padding-left:${vo.depth*20}px">
									...
									<a href="${pageContext.request.contextPath }/board/view/${vo.no }/${param.page}">${vo.title }</a>
								</td>
								<td>${vo.userName }</td>
								<td>${vo.hit }</td>
								<td>${vo.regDate }</td>
						...
						<c:if test="${totalPage > 5 }">
							<c:choose>
								<c:when test="${param.page == 1 }">
									<li><a href="${pageContext.request.contextPath }/board?page=1&kwd=${kwd }">◀</a></li>
								</c:when>
								<c:otherwise>
									<li><a href="${pageContext.request.contextPath }/board?page=${param.page-1 }&kwd=${kwd }">◀</a></li>
								</c:otherwise>
							</c:choose>
						</c:if>
						<c:choose>
						...
						<c:if test="${totalPage > 5 }">
							<c:choose>
								<c:when test="${param.page == totalPage }">
									<li><a href="${pageContext.request.contextPath }/board?page=${param.page }&kwd=${kwd }">▶</a></li>
								</c:when>
								<c:otherwise>
									<li><a href="${pageContext.request.contextPath }/board?page=${param.page+1 }&kwd=${kwd }">▶</a></li>
								</c:otherwise>
				...
				<div class="bottom">
					<c:if test="${!empty authUser }">
						<a href="${pageContext.request.contextPath }/board/write?page=${param.page}&kwd=${kwd }" id="new-book">글쓰기</a>
					</c:if>
				</div>				
			</div>
		</div>
